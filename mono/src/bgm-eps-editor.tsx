...
function initEditor<T extends Record<string, string>>(setting: EditorSetting<T>) {
  ...
  function bindEvents() {
    ...
    function applyState(state: string) {
      ...
      render(<EpList current={state} pushState={pushState} popState={popState} />, tableContainer);
  ...
  const encodeEpisodes = (decodeEPs: T[]) =>
    decodeEPs
      .map(e => columns.map(c => e[c] || "").join("|"))
      .join("\n");

  const decodeEpisodes: (text: string) => T[] = text =>
    text
      .split(/\n+/)
      .map(l => l.trim())
      .filter(l => l)
      .map(l => {
        ...
        return values.reduce((row, v, index) => ((row[columns[index]] = v as T[keyof T]), row), {} as T);
      });
  ...
  class EpList extends Component<EpListProps, never> {
    th() {
      return (
        <tr>
    ...
    onPaste = (row: number, field: keyof T) => (ev: ClipboardEvent) => {
      ...
      for (let r = row; r < eps.length && lines.length; r++) {
        const ep = eps[r];
        ep[field] = lines.shift() as T[keyof T];
      }
    ...
    onInput = (row: number, field: keyof T) => (ev: Event) => {
      ...
      const eps = this.decodeEPs();
      eps[row][field] = newText as T[keyof T];
      this.props.pushState(encodeEpisodes(eps));
    };
