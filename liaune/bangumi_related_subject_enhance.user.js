...
`);
  ...
  // 获取本地缓存
  const getCache = (itemId, callback) => {
    let request = indexedDB.open(dbName, 1);
    request.onsuccess = (evt) => {
      let db = evt.target.result;
  ...
  const createElement = (type, className, href, textContent) => {
  ...
  const displayCollect = (interest, elem) => {
    let avatarNeue = elem.querySelector("span.avatarNeue");
    if (interest == "wish") {
  ...
  if (volumeSubjects.length) {
    ...
    allCollect.onclick = () => {
      ...
      let getVolumeSubjects = setInterval(() => {
