...
if($('.idBadgerNeue a.avatar').attr('href').search($('.grp_box a.avatar').attr('href')) >= 0) {
    ...
    $('#indexCatBox ul').append('<li><a id="addRelateBatch" class="add thickbox" title="批量添加" href="#TB_inline?tb&height=300&width=420&inlineId=newIndexRelatedBatch"><span>+批量添加</span></a></li>');
    $('<div style="display:none;" id="newIndexRelatedBatch"><div class="bibeBox" style="padding:10px"><label>输入条目URL，如 http://bgm.tv/subject/265 或 http://bgm.tv/ep/292912，一行一个</label><textarea rows="15" class="quick" name="urls"></textarea><input type="button" class="inputBtn" value="批量添加关联" name="submit" onclick="addRelateBatch()"></div></div>').insertBefore('#indexCatBox');
...
$('#modifyOrder').click(function() {
    ...
    $('#savenowOrder').click(function() {
        ...
        $('#browserItemList > li,#columnSubjectBrowserA .browserList > li,#columnSubjectBrowserA .browserCrtList > div').each(function(i) {
            ...
            saveRelateItem(itemid1, content1, i);
            totalItems++;
        });
    });
});
...
window.addRelateBatch = function() {
    ...
    for(var i = 0;i < items.length; i++) {
            $.post(url, {add_related: items[i], formhash: formhash, submit: '添加新关联'},function() {
