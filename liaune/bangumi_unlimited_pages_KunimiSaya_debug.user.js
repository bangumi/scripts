...
// @version      0.7
// @description  自动加载下页。国见佐彩修改：已支持目前(2025.11)人物、虚拟角色页面、维基人申请页面，“Stop Loading All Pages”按钮固定到窗口左下方
// @author       Liaune、国见佐彩
...
(function() {
    ...
    if( window == window.top && mainTable && bottomDoc && nextPage )
    {
        ...
        loadAllLink.onclick = function(){
            ...
            loadAllLink.setAttribute("style","position: fixed; bottom: 50px; left: 50px;");
            testScrollPosition();
        };
    ...
    function getMainTable(source){
        var result = null, tableFun =
            [
                //wiki/applicants
                function(src){ src = src.querySelector('ul.line_list.user_list'); return src ? src : null; },

                //user/mono/character and user/mono/person
                function(src){ src = src.querySelector('ul.browserCoverMedium'); return src ? src : null; },
                ...
                //pm
                function(src){ src = src.querySelector('#pm_main .topic_list tbody'); return src ? src : null; },
            ];
