...
const VERSION = "1.1.0";
const SEARCH_APIS_URL =
  "https://raw.githubusercontent.com/22earth/gm_scripts/master/searchapis.json";

if (GM_registerMenuCommand) {
  // 用户脚本命令清除缓存信息
  GM_registerMenuCommand(
    "获取最新搜索引擎列表",
    () => clearInfoStorage(USERJS_PREFIX),
    "f"
  );
}
...
async function getSearchAPIs(str) {
  ...
  if (
    !searchAPIsResource ||
    infoOutdated(USERJS_PREFIX, UPDATE_INTERVAL, VERSION)
  ) {
    console.log("begin fetch apis");
    searchAPIsResource = await gmFetch(SEARCH_APIS_URL);
    const myRules = JSON.parse(searchAPIsResource);
    const magnetWRules = await getMagnetWRule();
    const apis = Object.assign({}, magnetWRules, myRules);
    localStorage.setItem(str, JSON.stringify(apis));

    localStorage.setItem(USERJS_PREFIX + "VERSION", VERSION);
    localStorage.setItem(
      USERJS_PREFIX + "LATEST_UPDATE_TIME",
      new Date().getTime()
    );
    return apis;
...
async function getMagnetWRule() {
  const URL = "https://magnetw.app/rule.json";

  const rules = JSON.parse(await gmFetch(URL));
  const myRules = {};
  rules.forEach((obj) => {
    let url = obj.url;
    if (obj.paths && obj.paths.preset) {
      const preset = obj.paths.preset
        .replace("{k}", "{searchTerms}")
        .replace("{p}", "1");
      url = `${url}${preset}`;
...
async function init() {
  ...
  const allSearchEngineLists = [
    ["dmhy"], // CN
    ["google", "sukebei", "tokyotosho"], // JP
  ];

  if (
    !localStorage.getItem("searchEngines") ||
    typeof JSON.parse(localStorage.getItem("searchEngines")) !== "object"
  ) {
  ...
  var addSearchIcon = {
    init: function () {
      if (
        window.location.href.match("/subject/") &&
        document.getElementById("navMenuNeue").children[2].children[0]
          .className !== "focus chl"
      )
        this.addIcon1();
      else if (
        window.location.href.match("/anime|index|game|book|subject_search/")
      )
        this.addIcon2();
    },
    createLink: function (link) {
      ...
      searchIconImg.style.cssText =
        "display:inline-block;border:none;height:12px;width:14px;margin-left:2px";
      ...
      if (link.match(re)) {
        ...
        const iconURL = searchAPIs[domain][1];
    ...
    getJanpaneseName: function (title) {
      if (window.location.href.match(/subject_search/)) {
        ...
        else return title.getElementsByClassName("l")[0].textContent;
      }
    ...
    getLink: function (engineName, animeName) {
      return searchAPIs[engineName][2].replace(
        /\{searchTerms\}/,
    ...
    addIcon1: function () {
      ...
      if (h1) {
        for (var i = 0, len = searchEngines.length; i < len; i++) {
          ...
          if (
            allSearchEngineLists[0].indexOf(engineName) > -1 ||
            !animeName.length
          )
    ...
    addIcon2: function addSearchIcon2() {
      ...
      //    if (window.location.href.match(/subject_search/))
      for (
        var i = 0, len = document.getElementsByTagName("h3").length;
        i < len;
        i++
      ) {
  ...
  var searchSwitch = {
    ...
    isHomepge: function () {
      return window.location.pathname === "/" &&
        document.getElementById("columnTimelineInnerWrapper")
        ? true
        : false;
    },
    ...
    insertSearchEngineSwitch: function () {
      ...
      for (var i = 0; i < searchEngineLists.length; i += 1) {
        if (searchEngines.indexOf(searchEngineLists[i]) > -1) {
          ...
          div.appendChild(
            this.createSwitch(searchEngineLists[i], "engine-off")
          );
        }
      }
    },
    createSwitch: function (name, aclass) {
      ...
      a.addEventListener("click", function (e) {
        ...
        status.textContent =
          "已开启" +
          document.querySelectorAll(".engine-on").length +
          "个搜索引擎";
