...
;(function () {
    ...
    BANGUMI = {
      init: function init() {
        ...
        if (!scoreInfoDouban) {
          ...
          fetchDoubanDataBySearch(subjectInfo);
        } else {
  ...
  function fetchDoubanDataBySearch(subjectInfo) {
    searchSubject(subjectInfo).then(function (result) {
      if (result) {
        var siteScoreInfo = {
          site: '豆瓣评分',
        ...
        updateScoreOnBangumi(siteScoreInfo, 'DOUBAN');
      } else {
        throw new Error("Invalid douban fuse result");
      }
    }).catch(function (err) {
      console.log('err', err);
    });
    // @TODO 引入 searchDoubanSubject 2020-02-22
    function searchSubject(subjectInfo) {
      var cat = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '1002';
      ...
      var url = 'https://www.douban.com/search?cat=' + cat + '&q=' + encodeURIComponent(query);
      return searchSubjectHTML(url).then(function (info) {
        var list = dealRawHTML(info);
        var options = {
          threshold: 0.3,
          location: 0,
          distance: 50,
        ...
        if (subjectInfo.startDate && result.length > 1) {
          var year = new Date(subjectInfo.startDate).getFullYear();
          if (year) {
            options.keys = ['year'];
    ...
    function dealRawHTML(txt) {
      ...
      var items = $doc.querySelectorAll('.search-result > .result-list > .result > .content');
      var dealHref = function dealHref(href) {
        var urlParam = href.split('?url=')[1];
        if (urlParam) {
      ...
      return Array.prototype.slice.call(items).map(function (item) {
        var href = dealHref(item.querySelector('.title h3 > a').getAttribute('href'));
        var $ratingNums = item.querySelector('.rating-info > .rating_nums');
        var ratingsCount = 0;
        var averageScore = 0;
        if ($ratingNums) {
          ratingsCount = ($ratingNums.nextElementSibling.innerText.match(/\d+/) || [0])[0];
          averageScore = $ratingNums.innerText;
        }
        var subjectCast = item.querySelector('.subject-cast').innerText;
        return {
          subjectCast: subjectCast,
  ...
  function updateScoreOnBangumi(siteScoreInfo, site) {
    var tempList = window.location.pathname.match(/subject\/(\d*)/);
    if (!tempList) {
      throw 'invalid subject url on bangumi.tv';
    };
    BANGUMI.insertScoreInfo(siteScoreInfo);
    localStorage.setItem(USERJS_PREFIX + site + '_' + tempList[1], JSON.stringify({
      info: siteScoreInfo,
      date: new Date().getTime()
    }));
  }
