...
let extractHTMLData = (HTMLString, matchRegExp) => HTMLString.match(matchRegExp)[0];
...
let robotWork =  (sth, time) => {
    ...
    window.top.chiiLib.ukagaka.presentSpeech(`<p>${sth}</p><p>(本提醒将在${time}秒后关闭)</p>`);
    setTimeout(function () {
        window.top.chiiLib.ukagaka.toggleDisplay();
    }, time*1000);
};
...
let componentInit = () => {
    ...
    if (!localStorage.getItem('bgm_user_detail_by_yonjar')) {
        ...
        Promise.all(todo).then(w => {
            localStorage.setItem('bgm_user_detail_by_yonjar', JSON.stringify(user1));
            robotWork('初始化完成!', 3);
        ...
        if (elem.tagName.toUpperCase() === 'BUTTON') {
            ...
            switch (elem.getAttribute('data_toUpdate')) {
                case 'characters':
                    ...
                    user.fetchChars()
                        .then(msg => {
                            localStorage.setItem('bgm_user_detail_by_yonjar', JSON.stringify(user));
                            elParent.previousSibling.textContent = user.getCount('characters');
...
class User {
    ...
    fetchChars() {
        return new Promise((resolve, reject) => {
            ...
            fetchHTMLString(url)
                .then(text => {
                    let domStr = extractHTMLData(text, /<div class="page_inner">[\S\s]*?<\/div>/);
                    ...
                    Promise.all(pagesArray)
                        .then(pages => {
                            ...
                            pages.forEach((page, i) => {
                                ...
                                domStr += extractHTMLData(page, /<ul class="coversSmall">[\S\s]*?<\/ul>/);
    ...
    fetchFriends() {
        return new Promise((resolve, reject) => {
            ...
            fetchHTMLString(url)
                .then(text => {
                    let domStr = extractHTMLData(text, /<ul id="memberUserList" class="usersMedium">[\S\s]*?<\/ul>/);
                    let dom = parseToDOM(domStr);
    ...
    fetchGroups() {
        return new Promise((resolve, reject) => {
            ...
            fetchHTMLString(url)
                .then(text => {
                    let domStr = extractHTMLData(text, /<ul id="memberGroupList" class="browserMedium">[\S\s]*?<\/ul>/);
                    let dom = parseToDOM(domStr);
                    let li = [...dom.querySelectorAll('a.avatar')];
