  ...
  async function loadComments(tl) {
    ...
    try {
      if (!comment || epCommentsCache.has(epId)
        ...
        comment = {
          html: unsafeWindow.bbcodeToHtml(content),
          id,
      ...
      commentBlock.className = 'comment netabare-comment-container';
      commentBlock.role = 'button';
      commentBlock.tabIndex = '0';
      commentBlock.innerHTML = `<span class="netabare-comment">${html}</span>`;
      card.before(commentBlock);
      unsafeWindow.renderBMO?.(commentBlock);
