...
(async function () {
    ...
    script.onload = () => {
        regions.forEach(from => {
            regions.forEach(to => {
                if (from !== to) {
                    converters[`${from}-${to}`] = OpenCC.Converter({ from, to });
                }
            });
        });
    };
    ...
    // 获取所有转换结果的辅助函数
    const getConvertedNames = (str) => {
        const converted = new Set();
        regions.forEach(from => {
            regions.forEach(to => {
                if (from !== to && converters[`${from}-${to}`]) {
                    converted.add(converters[`${from}-${to}`](str));
                }
            });
        });
    ...
    if (location.pathname.match(/^\/ep\/\d+$/)) {
        ...
        btn.addEventListener('click', async () => {
            try {
                ...
                const epsWithNoMatches = allEpsWithDesc.filter(
                    epLabel => !epLabelRoleMap[epLabel]
                );
...
function updAppearEps(epLabelRoleMap, subjectId, epsWithNoMatches) {
    ...
    epLabelRoleMapEntries.forEach(([epLabel, personRoleMap]) => {
        Object.entries(personRoleMap).forEach(([name, roles]) => {
            roles.forEach(role => {
                const matches = name => li => {
                ...
                let matchedLi = Array.from(crtLiList).find(matches(nameWithoutBrackets));
                let finalName = nameWithoutBrackets;

                if (!matchedLi) {
                    const converted = getConvertedNames(nameWithoutBrackets);
                    for (const name of converted) {
                        matchedLi = Array.from(crtLiList).find(matches(name));
                        if (matchedLi) { finalName = name; break; }
                    }
                }
                ...
                if (!matchedLi) {
                    const converted = getConvertedNames(name);
                    for (const name of converted) {
                        matchedLi = Array.from(crtLiList).find(matches(name));
                        if (matchedLi) { finalName = name; break; }
                    }
                ...
                // 使用最终匹配成功的名称生成groupKey
                const groupKey = `${finalName}-${role}`;

                if (matchedLi) {
                    const liId = `staff-${finalName.replace(/\s/g, '')}-${role.replace(/\s/g, '')}`;
                    matchedLi.id = liId;
                    ...
                    if (!groupedRecords[targetGroup][groupKey]) {
                    ...
                    if (!groupedRecords.unmatched[groupKey]) {
                        groupedRecords.unmatched[groupKey] = { name, role, epLabels: new Set() };
                    }
