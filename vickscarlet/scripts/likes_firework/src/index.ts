;(() => {
    ...
    const displacement = (v0: number, a: number, t: number) => v0 * t + (a * t * t) / 2
    ...
    class Launch {
        ...
        acceleration = 4 / 100
        time = Date.now()
        constructor({ start, target, hue, end }: LaunchProps) {
            this.start = Array.from(start) as Pos
            this.target = Array.from(target) as Pos
            this.current = Array.from(this.start) as Pos
            this.hue = hue
            this.end = end
        ...
        update(ctx: CanvasRenderingContext2D, _: number) {
    ...
    class Explosion {
        ...
        _dp = false
        _dl = false
        constructor({ center, hue, url, end }: ExplosionProps) {
    ...
    class Like {
        ...
        static readonly data = new Map<string, number[][]>()
        static async init(urls: string[]) {
        ...
        static async loading(url: string) {
    ...
    class Particle {
        ...
        lineWidth = 1
        flickerDensity = 20
        constructor({ center, hue, end }: ParticleProps) {
    ...
    type FireworkState =
        | { type: 'idle' | 'end' }
        | { type: 'launch'; item: Launch }
        | { type: 'explosion'; item: Explosion }
