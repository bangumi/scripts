import { Database, type BaseData } from '@b38dev/database'
...
export async function updateDatabase() {
    ...
    const lastVersion = (await db.get<database.VersionValue>('values', 'version'))?.version || 0
    if (lastVersion < 5) {
        // V5 update
        ...
        for (const { id, names, namesUpdate: update, namesTml: tml, block, note, tags } of users) {
            ...
            if (
                !block &&
                !note &&
                !(tags && ((tags as Set<string>).size || (tags as string[]).length))
            ) {
