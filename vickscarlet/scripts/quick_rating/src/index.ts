...
;(() => {
    ...
    observeChildren(
        ...
        (item: Element) => {
            ...
            const modify = item.querySelector<HTMLAnchorElement>('.collectModify > a:last-child')
            if (!modify) return
            const gh = modify.getAttribute('onclick')!.match(/\(\d+\s*,\s*'(.*)'\s*\)/)?.[1]
            if (!gh) return
            let info = item.querySelector('.collectInfo')
            if (!info) return
            const starstop = info.querySelector<HTMLAnchorElement>('.starstop-s')
            if (starstop) starstop.style.display = 'none'
            const id = item.id.substring(5)
            const star = (parseInt(starstop?.children[0].className.substring(15)!) || 0) + ''
