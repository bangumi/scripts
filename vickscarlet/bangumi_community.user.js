...
(async () => {
    ...
    function hoverUserListener() {
        ...
        let timeout;
        Event.on('hover', async ({ id, currentName }) => {
    ...
    function parseHasCommentList() {
        ...
        const handlerClearit = async clearit => {
            ...
            clearit.addEventListener('mouseenter', e => {
                Event.emit('hover', { id, currentName })
                e.stopPropagation();
            });
            clearit.addEventListener('mouseleave', () => Event.emit('leave', { id, currentName }));
            if (data.block) {
