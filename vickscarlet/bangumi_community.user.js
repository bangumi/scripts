...
(async () => {
    ...
    class Menu {
        ...
        async #show() {
            ...
            const tags = create('fieldset', { class: ['tags', 'board', 'loading', 'editable', 'tags-field'] }, ['legend', svg('tag'), '标签']);
            const note = create('fieldset', { class: ['note', 'board', 'loading', 'editable'] }, ['legend', svg('note'), '备注']);
            ...
            await Promise.all([
                ...
                async () => {
                    // 标签
                    const e = tags;
                    const edit = create('div', { class: ['svg-icon', 'action', 'normal'] }, svg('edit'), ['span', '编辑']);
                    const ok = create('div', { class: ['svg-icon', 'action', 'edit'] }, svg('ok'), ['span', '保存']);
                    const close = create('div', { class: ['svg-icon', 'action', 'edit'] }, svg('close'), ['span', '取消']);
    ...
    function svg(type, size = 14) {
        ...
        switch (type) {
            ...
            case 'ok': return ['svg', { viewBox: '3 3 18 18', fill, ...baseParams },
                ['path', { d: 'M19.3,5.3L9,15.6l-4.3-4.3l-1.4,1.4l5,5L9,18.4l0.7-0.7l11-11L19.3,5.3z' }],
            ];
            case 'close': return ['svg', { viewBox: '4 4 16 16', fill, ...baseParams },
