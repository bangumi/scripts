...
(async () => {
    ...
    const menu = new class {
        ...
        async #loadUserData(id) {
            ...
            if (namesUpdate < Date.now() - 3600_000) return data;
            ...
            data.names = new Set(ret).union(names);
            data.names.delete('');
            if (namesTml && names.size == data.names.size) return data;
            data.namesUpdate = Date.now();
            data.namesTml = tml;
            await db.put('users', data);
            return data
        }
        ...
        async #loadHomepage(id) {
            ...
            return { name, src, bio, nid, gh, friend }
        }
        ...
        async #show() {
            ...
            const homeBtn = create('div', { class: ['home', 'svg-icon'], onClick: () => newTab('/user/' + id) }, svg('home'), ['span', '主页']);
            append(addition, homeBtn);
            append(document.body, [this.#panel, ['div', { class: 'container' }, avatar, tags, note, usedname, bio, addition]]);
            ...
            (async () => {
                ...
                const { name, src, friend, nid, gh } = homepage;
                bio.classList.add(this.#bfbgi(src))
                append(avatar, ['img', { src }], createTextSVG(name, 'vc-serif'), ['span', id]);
                append(bio, homepage.bio);
                const pmBtn = create('div', { class: ['pm', 'svg-icon'], onClick: () => newTab('/pm/compose/' + nid + '.chii') }, svg('message'), ['span', '私信']);
                const connectBtn = create('div', { class: ['friend', 'svg-icon'] }, svg('connect'), ['span', '加好友']);
                const disconnectBtn = create('div', { class: ['friend', 'svg-icon'] }, svg('disconnect'), ['span', '解除好友']);
                ...
                append(addition, pmBtn, homeBtn, friend ? disconnectBtn : connectBtn);
                if (homepage.bio) $(homepage.bio).niceScroll(nicescroll);
            })();
            (async () => {
                // 加载 曾用名
                ...
                const usednameUl = this.#ul(user.names);
                const tagsUl = this.#ul(user.tags);
                append(usedname, usednameUl);
                append(tags, tagsUl, editTags);
                append(note, user.note ?? '', editNote);
                const blockedBtn = create('div', { class: ['block', 'svg-icon'] }, svg('block'), ['span', '解除屏蔽']);
                const unblockBtn = create('div', { class: ['block', 'svg-icon'] }, svg('notify'), ['span', '屏蔽']);
                ...
                append(addition, user.block ? blockedBtn : unblockBtn);
