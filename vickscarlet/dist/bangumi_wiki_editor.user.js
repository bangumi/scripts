...
(function () {
  ...
  function agentWithMonaco(element, init, onChange) {
    ...
    monaco.languages.registerCompletionItemProvider("bangumi-wiki", {
      provideCompletionItems: (model2, position) => {
        var word = model2.getWordUntilPosition(position);
        var range = {
  ...
  async function main() {
    ...
    resizeable.style.display = element.getAttribute("style")?.includes("display: none") ? "none" : "flex";
