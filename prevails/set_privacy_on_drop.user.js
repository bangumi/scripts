...
document.querySelectorAll('#dropped, #SecTab [onclick*=dropped]').forEach(
    i => i.addEventListener('click', () => setPrivacy(true)));

const nonDropSelector = ['wish', 'collect', 'do', 'on_hold'].map(s => `#${s}, #SecTab [onclick*=${s}]`).join(', ');
document.querySelectorAll(nonDropSelector).forEach(i => i.addEventListener('click', () => setPrivacy(remain)));
